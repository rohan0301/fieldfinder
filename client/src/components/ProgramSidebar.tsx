...
function ProgramTypeTag({ type }: { type: Program['orgType'] }) {
  const styles: Record<Program['orgType'], { bg: string; color: string; border: string }> = {
    'rbi': { bg: 'rgba(232,168,56,0.15)', color: '#E8A838', border: 'rgba(232,168,56,0.3)' },
    'little-league': { bg: 'rgba(74,144,217,0.15)', color: '#4A90D9', border: 'rgba(74,144,217,0.3)' },
    'parks-rec': { bg: 'rgba(39,174,96,0.15)', color: '#27AE60', border: 'rgba(39,174,96,0.3)' },
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              {program.status === 'confirmed-active' ? (
                <span className="flex items-center gap-1 text-xs" style={{ color: '#27AE60', fontFamily: "'Inter', sans-serif", fontSize: '11px' }}>
                  <CheckCircle size={10} />
                  Confirmed Active
                </span>
              ) : (
          ...
          <div className="flex gap-4">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span style={{
                ...
                fontSize: '15px',
                color: program.cost === 'free' ? '#27AE60' : program.cost === 'low-cost' ? '#E8A838' : '#F7F5F0',
              }}>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span style={{
                fontFamily: "'JetBrains Mono', monospace",
                fontSize: '15px',
                color: program.equipmentProvided ? '#27AE60' : 'rgba(247,245,240,0.6)',
              }}>
        ...
        {distance !== null && (
          <div className="px-5 py-3" style={{ borderBottom: '1px solid rgba(247,245,240,0.08)' }}>
            <div className="flex items-center gap-2">
              <Navigation size={13} style={{ color: '#4A90D9' }} />
              <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '13px', color: '#4A90D9' }}>
                {distance < 1
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  ...
                  style={{
                    background: program.registrationOpen ? 'rgba(39,174,96,0.15)' : 'rgba(247,245,240,0.08)',
                    color: program.registrationOpen ? '#27AE60' : 'rgba(247,245,240,0.45)',
                    border: `1px solid ${program.registrationOpen ? 'rgba(39,174,96,0.3)' : 'rgba(247,245,240,0.12)'}`,
        ...
        <div className="px-5 py-3" style={{ borderBottom: '1px solid rgba(247,245,240,0.08)' }}>
          ...
          <div className="flex flex-col gap-2">
            {program.phone && (
              ...
              >
                <Phone size={12} style={{ color: '#4A90D9' }} />
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '13px', color: '#4A90D9' }}>
                  {program.phone}
                </span>
              </a>
            ...
            {program.website && (
              <a
                ...
                className="flex items-center gap-2 hover:opacity-80 transition-opacity"
              >
                <Globe size={12} style={{ color: '#4A90D9' }} />
                <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '13px', color: '#4A90D9' }}>
                  Visit website
                </span>
              </a>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3" style={{ borderBottom: '1px solid rgba(247,245,240,0.08)' }}>
        ...
        >
          <Filter size={13} style={{ color: '#E8A838' }} />
          <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '13px', color: '#E8A838', letterSpacing: '0.05em' }}>
            FILTERS
          </span>
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  style={{
                    background: !filters.costFreeOnly ? 'rgba(232,168,56,0.2)' : 'rgba(247,245,240,0.06)',
                    color: !filters.costFreeOnly ? '#E8A838' : 'rgba(247,245,240,0.5)',
                    border: `1px solid ${!filters.costFreeOnly ? 'rgba(232,168,56,0.4)' : 'rgba(247,245,240,0.1)'}`,
                    fontFamily: "'Inter', sans-serif",
                    fontSize: '12px',
                  }}
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs transition-all"
                  style={{
                    background: filters.costFreeOnly ? 'rgba(39,174,96,0.2)' : 'rgba(247,245,240,0.06)',
                    color: filters.costFreeOnly ? '#27AE60' : 'rgba(247,245,240,0.5)',
                    border: `1px solid ${filters.costFreeOnly ? 'rgba(39,174,96,0.4)' : 'rgba(247,245,240,0.1)'}`,
                    fontFamily: "'Inter', sans-serif",
                    fontSize: '12px',
                  }}
                >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(['all', 'rbi', 'little-league', 'parks-rec', 'nonprofit'] as const).map((type) => (
                  <button
                    key={type}
                    onClick={() => onFilterChange({ ...filters, programType: type })}
                    className="px-3 py-1.5 rounded-lg text-xs transition-all"
                    style={{
                      background: filters.programType === type ? 'rgba(232,168,56,0.2)' : 'rgba(247,245,240,0.06)',
                      color: filters.programType === type ? '#E8A838' : 'rgba(247,245,240,0.5)',
                      border: `1px solid ${filters.programType === type ? 'rgba(232,168,56,0.4)' : 'rgba(247,245,240,0.1)'}`,
                      fontFamily: "'Inter', sans-serif",
                      fontSize: '12px',
                    }}
                  >
                    {type === 'all' ? 'All' : getProgramTypeLabel(type)}
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="px-5 py-3" style={{ borderTop: '1px solid rgba(247,245,240,0.1)' }}>
        ...
        <div className="flex flex-col gap-1.5">
          {[
            { color: '#E8A838', label: 'RBI Affiliate' },
            { color: '#4A90D9', label: 'Little League' },
            { color: '#27AE60', label: 'Parks & Rec' },
